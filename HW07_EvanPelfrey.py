...
comment after the code (in the form of English
sentences, not Python code).
'''
...
def litCricFriend(wordList, text):
    ...
    listOfWords = spaceReplace.split()  
    ...
    # words in wordList. Up to five lines of code. (It depends on your
    # coding style -- various styles are OK.)
    counter = 0
    for word in wordList:
        x = splitList.count(word)
        counter += x
...
# 'the' in each of the two poems above. Print out the value returned by
# each function call, identifying what it is. For example, it might say
bellsAAnFrequency =(litCricFriend(['a', 'an'], theBells))
print(bellsAAnFrequency)
bellsTheFrequency =(litCricFriend(['the'], theBells))
print(bellsTheFrequency)
cantoXIIAAnFrequency = (litCricFriend(['a', 'an'], cantoXII))
print(cantoXIIAAnFrequency)
cantoXIITheFrequency = (litCricFriend(['the'], cantoXII))
print(cantoXIITheFrequency)
...
'''
Poe uses 'a' and 'an' differently because the ratios are different by about .025, and they use 'the' differently because the ratios are different by .08
'''
